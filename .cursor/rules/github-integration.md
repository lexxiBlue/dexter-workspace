...
- **Prioritize**: `dexter.sql`, `schema.sql`, `helpers/`, active branch diffs
...
- **Cache**: Enable long-context caching for `dexter.sql` (saves tokens ~90%)
...
2. Include schema context if DB-related (`dexter.sql` relevant sections)
...
- Pin `dexter.sql` + `schema.sql` in chat context (reusable across requests)
...
4. **Pin your schema**
   - Start every session: "Context: [paste dexter.sql]"
...
- [ ] `schema.sql` + `dexter.sql` in Cursor project root
